...
public class DataGrid {
     ...
     *
     * @param input The input array. It must have at least two rows, at least
     * two columns, and at least as many rows as columns.
     *
     *
     */
    public DataGrid(double[][] input) {
    ...
    /**
     * Extract the data for a given row and column from a Grid object.
     *
    ...
    /**
     * Calculuate the mean of an array This method returns the mean of an input
     * array.
     *
    ...
    /**
     * Calculuate the variance of an array This method returns the variance of
     * an input array. A sample-size correction is not used.
     *
     * @param inputArray The array for calculation
     *
    ...
    /**
     * Test if the Grid object has zero variance. An important property to avoid
     * in a data set for machine learning is columns with zero variance. These
     * are columns that have only one unique value. This can be detected by
     * finding columns with no variance.
     *
     * @return Whether the Grid object has at least one column with zero
     * variance
     *
     *
     */
    ...
    /**
     * Standardize the values in a grid. This method standardizes the numbers in
     * a grid by centering each column around zero (i.e. forcing the column
     * means to be zero) and giving each column unit variance (i.e. forcing the
     * column variances to be one). This is a necessary operation for many
     * algorithms that rely on distances measures so that the results of the
     * algorithm are independent of the scale of the input data and units in
     * which they are expressed. Note: if any column in the Grid object has zero
     * variance, the scaling operation will not be performed.
     *
    ...
    /**
     * Calculuate the pair-wise distances between all points on the output
     * Kohonen grid. The maximum, rectilinear distance is used with a
     * rectangular geometry grid. The method works on a n * 2 Grid and produces
     * a n * n output Grid with the distances. For example. the first rows
     * contains the distances from the first pair in the implicit argument Grid
     * to all of the other points. The second row of the output Grid similarly
     * contains the distances to all the other points from the second point in
     * the implicit argumetn Grid, etc.
     *
     * @return A grid object representing the pairwise distances
     *
     *
     */
    public DataGrid distance() {
